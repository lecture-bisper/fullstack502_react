...
//  리액트는 Jquery 를 사용하지 않기 때문에 비동기 통신을 위한 axios 라이브러리가 필요함
...
//    예시 : get/delete 의 사용방법이 동일함, post/put 의 사용방법이 동일함

//    매개변수를 URL에 포함하여 axios 사용
//    > axios.get('http://localhost:8080/login?id=bitc&pw=1234')
//        .then(res => {성공 시 실행할 소스})
//        .catch(err => {실패 시 실행할 소스});

//    매개변수를 params 라는 속성에 추가하여 axios 사용
...
function AxiosTest () {
  ...
  const getKobisData = () => {
    axios.get('https://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=c55013eadce1f0005fae142c556a228d&targetDt=20250902')
      //  통신 성공 시 실행
      .then(res => {
        console.log('비동기 통신 성공!!');
        
        //  axios 를 통해서 전달받은 전체 데이터 가져오기
        //  axios 를 통해서 전달받은 데이터는 object 타입으로 전달됨
        //  config, data, headers, request, status, statusText 의 속성을 가지고 있음
        //  서버에서 전달한 실제 데이터는 'data' 속성에 담겨 있음
        // console.log(res);

        //  전달받은 전체 데이터 중 필요한 데이터만 객체 분할 할당 방식으로 데이터를 가져옴
